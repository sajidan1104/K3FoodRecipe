...
class DetailRecipeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (helper!!.cekData(modelFilter?.idMeal)) {
        ...
        imgFavorite.setOnFavoriteChangeListener { buttonView, favorite ->
            if (favorite) {
                idMeal = modelFilter?.idMeal
                strMeal = modelFilter?.strMeal
                strMealThumb = modelFilter?.strMealThumb
                helper?.addFavoriteRecipes(idMeal, strMeal, strMealThumb)
                Snackbar.make(buttonView, modelFilter?.strMeal + " Added to Favorite",
                    Snackbar.LENGTH_SHORT).show()
            } else {
                idMeal = modelFilter?.idMeal
                helper!!.deleteFavoriteRecipes(idMeal)
            Snackbar.make(buttonView, modelFilter?.strMeal + " Removed from Favorite",
                Snackbar.LENGTH_SHORT).show()
        }
        }
    }
